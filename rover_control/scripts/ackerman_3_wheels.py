...
class ZhurongMarsRoverControl(object):
	...
	def cmd_vel_callback(self, msg):
		# print('received!!!')
		self.body_velocity = msg.linear.x*2/3
		self.body_omega = msg.angular.z*2/3
	...
	def init_publisher_variables(self):
		...
		self.steer_publisher.append(self.zhurong_publishers[self.controllers_list[-1]])
	...
	def init_state(self):
		self.set_turning_radius(np.zeros(3))
		self.set_wheels_speed(np.zeros(3))

	def set_turning_radius(self, turn_radius):
		self.wheel_steer_msg[0].data = turn_radius[2]
	...
	def move_with_cmd_vel(self):
		if self.body_omega == 0:
			...
			vel_arr = np.ones(3) * self.body_velocity/self.r
			self.set_wheels_speed(vel_arr)
		else:
			turning_radius = self.body_velocity/self.body_omega
			r_arr = np.zeros(3)
			r_arr[0] = abs(turning_radius-self.h)
			r_arr[1] = abs(turning_radius+self.h)
			r_arr[2] = np.sqrt(turning_radius**2+self.l**2)
			...
			theta[0] = 0
			theta[1] = 0
			theta[2] = -np.arctan(self.l/turning_radius)
